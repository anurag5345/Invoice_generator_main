...
export class InvoicesController {
  ...
  async downloadPdf(
    ...
    try {
      ...
      doc.text('Price', itemCols[3] + 5, tableTop + 7);
      ...
      (invoice.items ?? []).forEach((item: LineItem, idx: number) => { //change
        ...
        doc.text(String(item.rate), itemCols[2] + 5, tableTop + 7);
        doc.text(
        ...
        doc.text(`₹${amount.toFixed(2)}`, itemCols[5] + 5, tableTop + 7);
      ...
      doc.text(
        `Subtotal: ₹${Number(invoice.totalWithoutGST).toFixed(2)}`,
        summaryX + 10,
        totalsY,
      );
      totalsY += 18;
      doc.text(
        `Total GST: ₹${Number(invoice.totalGST).toFixed(2)}`,
        summaryX + 10,
        totalsY,
      );
      totalsY += 18;
      doc.text(
        `Total: ₹${Number(invoice.totalWithGST).toFixed(2)}`,
        summaryX + 10,
        totalsY,
      );
      totalsY += 18;
      doc
        .font('Helvetica-Bold')
        .text(
          `Amount Due: ₹${Number(invoice.totalWithGST).toFixed(2)}`,
          summaryX + 10,
          totalsY,
        );
      ...
      (dto.items || []).forEach(
        (
          ...
          doc
            .fontSize(12)
            .text(
              `${idx + 1}. ${item.description} | Rate: ${item.rate} | Price: ₹${Number(item.unitPrice).toFixed(2)} | GST: ${Number(item.gstRate || 0).toFixed(2)}%`,
            );
        },
      );

      doc.moveDown();
      doc.text(`Total (without GST): ₹${Number(subtotal).toFixed(2)}`);
      doc.text(`Total GST: ₹${Number(totalGST).toFixed(2)}`);
      doc.text(`Total (with GST): ₹${Number(subtotal + totalGST).toFixed(2)}`);
